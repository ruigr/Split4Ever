var Widgets = (function(){
	...
	module.prototype.createItemWidget = function(item, container){
		...
		h5.textContent = '&euro; ' + item.price;
		...
		var counter = 0;
		var figures = container.find('#' + item._id);
		numOfFigures = figures.length
		...
		var spanN = container.find('.next','#' + item._id)[0];
		...
		var spanP = container.find('.prev','#' + item._id)[0];
	...
	module.prototype.createItemImageWidget = function(item, container){

		var outWrapper = document.createElement("div");
		$( container ).append(outWrapper);
		outWrapper.classList.add("col-sm-12");
		outWrapper.classList.add("col-md-6");
		outWrapper.setAttribute('id', 'col1');
